...
% TODO: confirm preferred name form before submission
\author{Chen-Ting Lin\thanks{\texttt{kiwi2002101517@gmail.com}}}
...
which had remained unproved since 2011--2018.  All computations are
...
family of such entries is due to R.~H.~Hardin, who between 2010 and 2018
...
later by C.~Barker) have remained open.  We prove all of them.
...
enumeration and the profile method), which agree with each other and
reproduce every term published in the OEIS entries (22--43 terms per
entry).  The polynomial coefficients are recovered by exact rational
interpolation.  In addition, the defining computation has been
implemented in the Lean~4 proof assistant, where concrete values such as
$a_{3,2}(3)=44$ are certified by \texttt{native\_decide}; full
formalization of Theorem~\ref{thm:main} is in progress.  Code and Lean
...
Below $n_0$ the polynomial genuinely fails (e.g.\ $a_{10,2}(3) = 0 \ne
P_{10,2}(3)$): for small $n$ the clipped neighborhoods of opposite faces
overlap and the census of Lemma~\ref{lem:census} no longer applies.  That
the true threshold $n_0$ is smaller than our guaranteed $2k-1$ reflects
...
The two-dimensional case has been fully formalized in Lean~4 with
mathlib: the locality lemma, translation invariance, the single-axis
census, and the polynomial form of Theorem~\ref{thm:main} for $d=2$ are
proved as theorems (depending only on Lean's standard axioms), and the
eight statements A188148--A188155 are machine-checked end to end, with
the window constants and sub-threshold values evaluated by the compiler
via \texttt{native\_decide} (adding only the standard
\texttt{Lean.ofReduceBool} trust assumption).  For example, the Lean
statement of A188148 reads: for all $n \ge 2$,
$(A\,2\,n : \mathbb{Z}) = 12n^2 - 24n + 8$, where $A\,m\,n$ is the
walk count defined from first principles.  The three- and
four-dimensional cases follow the same blueprint and their formalization
is in progress.  In addition, an independent list-based Lean
implementation of the count is checked by \texttt{native\_decide}
...
Electron.\ J.\ Combin.\ (2024); arXiv:2309.00487.
% TODO: verify volume/article number before submission
...
graphs}, Aequationes Math.\ (2024).
% TODO: verify volume/pages before submission

\bibitem{JCTA}
\emph{Step-constrained self-avoiding walks on finite grids},
J.\ Combin.\ Theory Ser.\ A (2025).
% TODO: fill in authors and article number before submission
